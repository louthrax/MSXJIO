...
def clean_iar_asm(input_path, output_path):
    ...
    for line in lines:
        ...
        if stripped.startswith("RSEG"):
            continue
        if stripped.startswith("DEFS"):
            continue

        if stripped.startswith("PUSH	BC"):
            continue

        if stripped.startswith("PUSH	DE"):
            continue
        
        if stripped.startswith("POP	DE"):
            continue

        if stripped.startswith("POP	BC"):
            continue

        #line = re.sub(r"^\s*DEFB\s*'([^']*)'", r'DEFM "\1"', line)
